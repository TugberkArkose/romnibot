...
class Launchpad_Class(object):
	...
	def __init__(self):
		...
		self._right_encoder_value = 0
                self._back_encoder_value = 0

		self._battery_value = 0
                
		self._north_ultrasonic_value = 0
                self._north_east_ultrasonic_value = 0
                self._north_west_ultrasonic_value = 0
                self._east_ultrasonic_value = 0
                self._west_ultrasonic_value = 0
                self._south_ultrasonic_value = 0
...
#Subscribers and Publishers
		...
		self._Right_Encoder = rospy.Publisher('rwheel',Int64,queue_size = 10)
                self._Back_Encoder = rospy.Publisher('bwheel',Int64,queue_size = 10)
		...
		self._US_North_Value = rospy.Publisher('us_north',Float32,queue_size = 10)
                self._US_North_East_Value = rospy.Publisher('us_north_east',Float32,queue_size = 10)
                self._US_North_West_Value = rospy.Publisher('us_north_west',Float32,queue_size = 10)
                self._US_East_Value = rospy.Publisher('us_east',Float32,queue_size = 10)
                self._US_West_Value = rospy.Publisher('us_west',Float32,queue_size = 10)
                self._US_South_Value = rospy.Publisher('us_south',Float32,queue_size = 10)
		...
		self._right_motor_speed = rospy.Subscriber('right_wheel_speed',Float32,self._Update_Right_Speed)
                self._back_motor_speed = rospy.Subscriber('back_wheel_speed',Float32,self._Update_Back_Speed)
	...
	def _Update_Left_Speed(self, left_speed):
		...
		speed_message = 's %d %d %d\r' %(int(self._left_wheel_speed_),int(self._right_wheel_speed_),int(self._back_wheel_speed_))
                #speed_message = 's w\r'
	...
	def _Update_Right_Speed(self, right_speed):
        ...
        def _Update_Back_Speed(self, back_speed):

		self._back_wheel_speed_ = back_speed.data

		rospy.loginfo(back_speed.data)
	...
	def _HandleReceivedLine(self,  line):
		...
		if(len(line) > 0):
			...
			try:
				...
				if(lineParts[0] == 'u'):
                                         self._north_ultrasonic_value = float(lineParts[1])
                                        # self._north_east_ultrasonic_value = float(lineParts[2])
                                        # self._north_west_ultrasonic_value = float(lineParts[3])
                                        # self._east_ultrasonic_value = float(lineParts[4])
                                        # self._west_ultrasonic_value = float(lineParts[5])
                                        # self._south_ultrasonic_value = float(lineParts[6])

                                         self._US_North_Value.publish(self._north_ultrasonic_value)
                                        # self._US_North_East_Value.publish(self._north_east_ultrasonic_value)
                                        # self._US_North_West_Value.publish(self._north_west_ultrasonic_value)
                                        # self._US_East_Value.publish(self._east_ultrasonic_value)
                                        # self._US_West_Value.publish(self._west_ultrasonic_value)
                                        # self._US_South_Value.publish(self._south_ultrasonic_value)
